...
class NodeEvent(Base):
    ...
    user = db.relationship(User, foreign_keys='NodeEvent.user_id')
...
def _new_event(host, port, event_domain, event_type, user, args):
    e = NodeEvent(
        host=host, port=port, event_domain=event_domain, event_type=event_type,
        args_json=json.dumps(args), user=user)
    db.session.add(e)
    db.session.flush()
    return e


def raw_event(host, port, event_type, user, args=''):
    return _new_event(host, port, None, event_type, user, args)


def eru_event(host, port, event_type, user, args=''):
    return _new_event(host, port, 'eru', event_type, user, args)
